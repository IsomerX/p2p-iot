...
const logger = createLogger("ControlClient");
...
class ControlClient extends EventEmitter {
  ...
  constructor(options: ControlClientOptions = {}) {
    ...
    this.options = {
      ...
      deviceName:
        options.deviceName || `ArrowTarget-${uuidv4().substring(0, 8)}`,
      discoveryPort: options.discoveryPort || DEFAULT_DISCOVERY_PORT,
      autoConnect:
    ...
    // Start discovery if auto-connect is enabled
  ...
  public startDiscovery(): void {
    ...
    try {
      ...
      // Bind to port
      this.discoverySocket.bind(this.options.discoveryPort, () => {
        logger.info(
          `Listening for controller announcements on port ${this.options.discoveryPort}`,
        );
  ...
  public stopDiscovery(): void {
    ...
    if (this.discoverySocket) {
      try {
        ...
        logger.error("Error closing discovery socket", error);
      }
      this.discoverySocket = null;
    }

    logger.info("Stopped controller discovery");
  }
      ...
      this.discoverySocket.send(
        ...
        (err) => {
          if (err) {
            ...
            logger.debug("Sent discovery request");
          }
        },
      );
    } catch (error) {
      logger.error("Error creating discovery request", error);
    }
  }
  ...
  private handleDiscoveryMessage(msg: Buffer, rinfo: dgram.RemoteInfo): void {
    try {
      // Parse message
      const { valid, message, error } = parseJsonMessage(msg.toString());
      if (!valid || !message) {
        logger.debug(`Invalid discovery message: ${error}`);
        return;
      }
      ...
      if (!validation.valid) {
        logger.debug(`Invalid discovery message: ${validation.error}`);
        return;
      }

      // Check message type
      if (message.type !== MESSAGE_TYPES.ANNOUNCE) {
        return;
      }

      // Check sender type
      if (message.sender.type !== DeviceType.CONTROLLER) {
        return;
      }

      // Extract controller info
      const { controllerInfo, discoveryPort, controlPort } = message.data;
      if (!controllerInfo || !controlPort) {
        logger.debug("Invalid controller announcement");
        return;
      }

      logger.info(
        `Discovered controller: ${controllerInfo.name} (${controllerInfo.id}) at ${rinfo.address}:${controlPort}`,
      );

      // Emit discovery event
      this.emit("controllerDiscovered", {
      ...
      if (
        ...
        this.connect(rinfo.address, controlPort).catch((error) => {
          logger.error("Error connecting to discovered controller", error);
        });
      }
    } catch (error) {
      logger.error("Error handling discovery message", error);
    }
  }
  ...
  private scheduleReconnect(): void {
    ...
    this.reconnectTimeoutId = setTimeout(() => {
      ...
      if (this.controllerInfo) {
        this.connect(this.controllerInfo.ip, this.controllerInfo.port).catch(
          (error) => {
